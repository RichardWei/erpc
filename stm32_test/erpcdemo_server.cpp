...
erpc_status_t DEMO_service::handleInvocation(uint32_t methodId, uint32_t sequence, Codec * codec, MessageBufferFactory *messageFactory, Transport * transport)
{
    ...
    switch (methodId)
    {
        case DEMO_interface::m_RD_demoHelloId:
        {
            erpcStatus = RD_demoHello_shim(codec, messageFactory, transport, sequence);
            break;
        }
...
erpc_status_t DEMO_service::RD_demoHello_shim(Codec * codec, MessageBufferFactory *messageFactory, Transport * transport, uint32_t sequence)
{
...
#endif
        result = m_handler->RD_demoHello(txInput);
#if ERPC_NESTED_CALLS_DETECTION
    ...
    if (err == kErpcStatus_Success)
    {
        ...
        codec->startWriteMessage(message_type_t::kReplyMessage, DEMO_interface::m_serviceId, DEMO_interface::m_RD_demoHelloId, sequence);
